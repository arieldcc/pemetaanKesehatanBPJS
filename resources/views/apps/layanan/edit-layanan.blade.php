...
<div class="row">
    <div class="col-md-6">
        <div class="card shadow mb-4">
            ...
            <div class="card-body ">
                <form method="POST" action="/data-layanan/{{ $layanan->id }}/update" enctype="multipart/form-data">
                    @csrf
                    <div class="form-row">
                        <div class="form-group col-md-12">
                            ...
                            <select class="js-example-basic-single form-control @error('fasilitas_kesehatan_id') is-invalid @enderror" id="fasilitas_kesehatan_id" name="fasilitas_kesehatan_id">
                                ...
                                @foreach ($fasilitas as $item)
                                    <option value="{{ $item->id }}" {{ $item->id==$layanan->fasilitas_kesehatan_id ? 'selected' : '' }}>{{ $item->nama_fasilitas.' | '.$item->alamat }}</option>
                                @endforeach
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-12">
                            ...
                            <input type="text" class="form-control @error('nama_layanan') is-invalid @enderror" placeholder="Nama atau Jenis Layanan" value="{{ $layanan->nama_layanan }}" name="nama_layanan" id="nama_layanan" disabled>
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-12">
                            ...
                            <textarea class="form-control @error('keterangan') is-invalid @enderror" rows="4" cols="4" name="keterangan" id="keterangan" disabled>{{ $layanan->keterangan }}</textarea>
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            if ($('#fasilitas_kesehatan_id').val() != "") {
                elmEnabled(false); // Jika tidak kosong, aktifkan semua elemen dengan elmEnabled(false)
                $.getJSON('/get-fasilitas/' + $('#fasilitas_kesehatan_id').val(), function(data) {
